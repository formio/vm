...
const flattenComponentsForRender = (data, components) => {
    ...
    FormioCore.Utils.eachComponent(
        ...
        (component, path) => {
            ...
            const autoAddress = util.isAutoAddress(data, component, path);
            const isDataArray =
...
const renderComponentValue = (data, key, components, noRecurse) => {
    ...
    if (
        ...
        return util.renderComponentValue(
    ...
    switch (component.type) {
        ...
        case 'container':
            ...
            _.each(value, (subValue, subKey) => {
                const subCompValue = util.renderComponentValue(
                    value,
                    subKey,
                    components,
                );
        ...
        case 'editgrid':
        case 'datagrid': {
            const columns = util.flattenComponentsForRender(
                data,
        ...
        case 'datetime': {
            ...
            compValue.value = moment(value).format(
                util.convertFormatToMoment(dateFormat),
            );
            break;
        }
        case 'radio':
...
util = {
  ...
  renderComponentValue: ${renderComponentValue.toString()},
}
`;
